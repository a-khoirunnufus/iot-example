(async function() {
    const tankHistoryMaster = await $.ajax({url: `${baseUrl}/api/tank-history?filters[tank]=${tankParam}`, async: true});
    ...
    const dateLabels = tankHistoryMaster.map(item => item.date);

    // console.log(tankData, dateLabels);
